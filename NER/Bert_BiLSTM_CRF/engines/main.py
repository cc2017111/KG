...
def check_fold(configures):
    ...
    if not hasattr(configures, checkpoints_dir) or not os.path.exists(os.path.join(base_path, configures.checkpoints_dir)):
        ...
        if len(paths) == 2 and os.path.exists(paths[0]) and not os.path.exists(os.path.join(base_path, configures.checkpoint_dir)):
            os.mkdir(os.path.join(base_path, configures.checkpoints_dir))
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Bert_BiLSTM_CRF_MODEL')
